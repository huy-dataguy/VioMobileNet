...
active_cameras = {} # Lưu trữ các process camera đang chạy

# --- API 1: UPLOAD VIDEO (OFFLINE) ---
...
def stop_camera(camera_id: str):
    ...
    # Gửi tín hiệu dừng qua Redis
    redis_client.set(f"stop_signal_{camera_id}", "1")
    
    # Đợi process tắt
    active_cameras[camera_id].join(timeout=5)
    if active_cameras[camera_id].is_alive():
        active_cameras[camera_id].terminate() # Kill nếu bị treo
